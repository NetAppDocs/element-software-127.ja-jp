...
[role="lead"]
メソッドを使用すると、クラスタ構成に新しい仮想ネットワークを追加できます `AddVirtualNetwork`。
...
NOTE: AddVirtualNetwork メソッドは、新しい仮想ネットワークを作成するためだけに使用されます。既存の仮想ネットワークに変更を加える場合は、メソッドを使用しxref:reference_element_api_modifyvirtualnetwork.adoc[ModifyVirtualNetwork]ます。
