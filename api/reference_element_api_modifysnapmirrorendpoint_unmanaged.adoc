...
[role="lead"]
Elementソフトウェアでは、このバージョンのメソッドを使用し `ModifySnapMirrorEndpoint`て、管理対象外のSnapMirrorエンドポイントのストレージクラスタ名またはIPアドレスの属性を変更します。管理対象外のエンドポイントは、 Element SnapMirror API では管理できません。ONTAP 管理ソフトウェアまたは API で管理する必要があります。
