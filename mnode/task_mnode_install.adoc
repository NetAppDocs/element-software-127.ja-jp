...
.. 追加するデフォルトタイムサーバーごとに新しい行を追加してください。デフォルトタイムサーバーは、<<set-up-the-management-node,後の手順>>で使用するストレージクラスターで使用されるNTPサーバーと同じである必要があります。
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [MVIPアドレス]*：Elementソフトウェアを実行しているストレージクラスタの管理仮想IPアドレス（MVIP）。管理ノードを、link:task_mnode_recover.html#configure-time-sync["NTP サーバの設定"]で使用したストレージクラスタと同じ構成にします。
