...
* 新しいNetApp HCCロールをlink:task_mnode_create_netapp_hcc_role_vcenter.html#add-an-asset-to-the-management-node["アセットを追加します"]管理ノードに割り当てます。
...
[NOTE]
====
*** NetApp Element Plug-in for vCenter Server 5.0以降でを使用するには、 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenterリンクモード"^]NetApp SolidFireストレージクラスタを管理するvCenter Serverごとに、別の管理ノードからElement Plug-inを登録します（推奨）。
*** を使用して他のvCenter Serverのクラスタリソースを管理するためにNetApp Element Plug-in for vCenter Server 4.10以前を使用 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenterリンクモード"^]できるのは、ローカルストレージクラスタのみです。
...
インストール後に新しいアセットを手動で追加する必要がある場合は、で作成した新しいHCCユーザーアカウントを使用し<<vCenter で新しいユーザアカウントを設定します>>ます。詳細については、を参照してください link:task_mnode_add_assets.html["管理ノードにコントローラアセットを追加します"]。
