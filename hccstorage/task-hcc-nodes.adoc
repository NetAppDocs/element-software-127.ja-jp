...
複数ノードシステムのストレージ情報を表示できます。が割り当てられている場合はlink:../concepts/concept_solidfire_concepts_data_protection.html#custom_pd["カスタムの保護ドメイン"]、特定のノードに割り当てられている保護ドメインを確認できます。
...
TIP: ストレージの数を確認するには、 NetApp Hybrid Cloud Control （ HCC ）ダッシュボードを参照します。を参照して link:task-hcc-dashboard.html["HCC ダッシュボードを使用してストレージリソースを監視する"]
