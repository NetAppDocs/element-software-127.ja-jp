...
| `storageNodeIds` | UUID の配列 | がに設定されて `true`いる場合 `storageLogs`は、このパラメータにストレージクラスタのノードIDを入力して、ログの収集対象を特定のストレージノードに限定します。使用可能なすべてのノードIDを確認するには、エンドポイントを使用し `GET https://[management node IP]/logs/1/bundle/options`ます。 
|===
